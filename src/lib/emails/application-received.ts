// ─────────────────────────────────────────────────────────────────────────
// Email templates for the "application received" flow.
//
// SINGLE SOURCE OF TRUTH for:
//   - cleanerWelcomeHtml — sent to the cleaner who just applied
//   - adminAlertHtml     — sent to the admin inbox (cleaners@vouchee.co.uk)
//
// Both the production route (/api/send-application-received) and the admin
// test route (/api/admin/test-application-emails) import from this file.
// A change here updates BOTH automatically — the test button always reflects
// what real cleaners will receive. There is no second copy to drift.
//
// To preview changes: edit a template here, push, then click the "Application
// received emails" button on the Admin → Tests tab.
// ─────────────────────────────────────────────────────────────────────────

// Brand colour — warm Vouchee blue. Used on all CTA buttons so they're
// consistent across welcome / approval / rejection / re-approval emails.
export const BRAND_BLUE = '#2563eb'

// Hosted logo PNG — same asset used by the customer/cleaner confirmation
// emails in /api/gocardless/confirm. Lives in /public, served by Next.js,
// renders correctly in Gmail/Outlook/Apple Mail.
export const LOGO_URL = 'https://www.vouchee.co.uk/full-logo-black.png'
...
// ═════════════════════════════════════════════════════════════════════════
export function cleanerWelcomeHtml(firstName: string, appUrl: string) {
