...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      requestId, cleanerId, customerId, message,
      cleanerName, cleanerDbs, cleanerInsured, cleanerRightToWork,
      jobZone, jobBedrooms, jobBathrooms, jobHours, jobRate,
    } = body
    ...
    // Fetch cleaner member since date
    const { data: cleanerProfile } = await supabase
      .from('profiles')
      .select('full_name, created_at')
      .eq('id', cleanerId)
      .single<{ full_name: string; created_at: string }>()

    const memberSince = cleanerProfile?.created_at
      ? new Date(cleanerProfile.created_at).toLocaleDateString('en-GB', { month: 'long', year: 'numeric' })
      : 'Recently'

    // Format cleaner name as "First L." for email
    const nameParts = cleanerName.trim().split(' ')
    const formattedName = nameParts.length > 1
      ? `${nameParts[0]} ${nameParts[nameParts.length - 1].charAt(0)}.`
      : nameParts[0]
    const cleanerInitial = nameParts[0].charAt(0).toUpperCase()

    // Fetch customer email via admin client (bypasses RLS)
    ...
    if (!customerProfile?.email) {
      console.error('Could not find customer email for notification')
      return NextResponse.json({ success: true, emailSent: false })
    }

    const customerFirstName = (customerProfile.full_name as string)?.split(' ')?.[0] ?? 'there'
    const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? 'https://www.vouchee.co.uk'

    const html = buildEmailHtml({
      customerFirstName,
      cleanerName: formattedName,
      cleanerInitial,
    ...
    const { error: emailError } = await resend.emails.send({
      ...
      to: customerProfile.email as string,
      subject: `New application from ${formattedName}`,
      html,
    })
...
function buildEmailHtml({
  customerFirstName, cleanerName, cleanerInitial, cleanerMemberSince,
  cleanerDbs, cleanerInsured, cleanerRightToWork,
  message, jobZone, jobBedrooms, jobBathrooms, jobHours, jobRate, appUrl,
}: {
  customerFirstName: string
  cleanerName: string
  cleanerInitial: string
  cleanerMemberSince: string
  cleanerDbs: boolean
          ...
          <table cellpadding="0" cellspacing="0"><tr>
            <td style="vertical-align:middle;">
              <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 411.55" width="32" height="32" style="display:block;">
                <defs>
                  <linearGradient id="a" x1="271.32" y1="378.46" x2="-1.11" y2="106.04" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#0350e6"/><stop offset="1" stop-color="#58c9fc"/></linearGradient>
                  <linearGradient id="b" x1="190.51" y1="395.38" x2="8.83" y2="79.68" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#0350e6"/><stop offset="0.05" stop-color="#0f60e9"/><stop offset="0.15" stop-color="#2580ef"/><stop offset="0.27" stop-color="#389bf4"/><stop offset="0.4" stop-color="#46aff7"/><stop offset="0.54" stop-color="#50befa"/><stop offset="0.72" stop-color="#56c6fc"/><stop offset="1" stop-color="#58c9fc"/></linearGradient>
                  <linearGradient id="c" x1="240.56" y1="261.33" x2="471.93" y2="29.96" xlink:href="#a"/>
                  ...
                  <linearGradient id="e" x1="315.29" y1="283.01" x2="501.98" y2="-66.17" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#0350e6" stop-opacity="0.5"/><stop offset="0.03" stop-color="#0350e6"/><stop offset="1" stop-color="#0350e6"/></linearGradient>
                </defs>
                <path fill="url(#a)" d="M0,102.22,160,382.46c21,36.86,73.31,39.15,97.49,4.28L315,303.8l-23.62,6.9a47.51,47.51,0,0,1-53.44-20.15L156,161.51a136.85,136.85,0,0,0-134-62.28Z"/>
                <path fill="url(#b)" d="M184.15,382.46l-160-280.24.84-.11c-.34-.4-.69-.8-1-1.23a8.84,8.84,0,0,1-1-1.77l-1,.12-22,3L160,382.46a57.56,57.56,0,0,0,62.24,27.83A57,57,0,0,1,184.15,382.46Z"/>
                <path fill="url(#c)" d="M220.61,238.18,302,104.12A215.68,215.68,0,0,1,481.56.41L500,0l-1.38,1.12A221.63,221.63,0,0,0,444,66.88L333.68,269.33A58.94,58.94,0,0,1,293.13,299h0a46.5,46.5,0,0,1-48.5-21.43Z"/>
                <path fill="url(#d)" d="M500,0l-.44.34-2.51.06A215.71,215.71,0,0,0,317.47,104.12l-81.36,134,24,39.38A46.37,46.37,0,0,0,292,299.15a45,45,0,0,1-7.74.66,46.46,46.46,0,0,1-39.66-22.26l-24-39.38,81.36-134A215.73,215.73,0,0,1,481.55.41Z"/>
                <path fill="url(#e)" d="M500,0l-1.38,1.11A221.69,221.69,0,0,0,444,66.87L333.67,269.33A58.81,58.81,0,0,1,293.13,299a46.48,46.48,0,0,1-8.83.84,45.69,45.69,0,0,1-10.72-1.26,58.84,58.84,0,0,0,38.58-29.22L422.5,66.87c10.27-19.15,30.33-48.5,65.39-62.75A110.21,110.21,0,0,1,500,0Z"/>
              </svg>
            </td>
        ...
        <tr><td style="background:white;border-radius:20px;padding:36px;border:1px solid #e2e8f0;">
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafc;border-radius:14px;padding:18px 20px;margin-bottom:24px;border:1px solid #e2e8f0;">
            ...
            <tr><td style="padding-bottom:14px;">
              ...
              ${jobHours ? `<span style="display:inline-block;background:white;border:1px solid #e2e8f0;border-radius:100px;padding:4px 12px;font-size:12px;font-weight:600;color:#475569;margin-right:4px;">${jobHours} hrs</span>` : ''}
              ${jobRate ? `<span style="display:inline-block;background:#fefce8;border:1px solid #fef08a;border-radius:100px;padding:4px 12px;font-size:12px;font-weight:700;color:#92400e;">£${jobRate}/hr</span>` : ''}
            </td></tr>
            <tr><td>
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="border:1.5px solid #e2e8f0;border-radius:16px;overflow:hidden;margin-bottom:20px;">
            <tr><td style="padding:20px 20px 16px;">
              <table width="100%" cellpadding="0" cellspacing="0"><tr>
                <td style="vertical-align:middle;">
                  <table cellpadding="0" cellspacing="0"><tr>
                    <td style="vertical-align:middle;">
                      <div style="width:52px;height:52px;border-radius:50%;background:#2563eb;text-align:center;display:table-cell;font-size:22px;font-weight:800;color:white;line-height:52px;">${cleanerInitial}</div>
                    </td>
            ...
            <tr><td style="padding:16px 20px;border-top:1px solid #f1f5f9;background:#fafafa;">
              ...
              <div style="padding:16px;background:#f8fafc;border-radius:10px;border:1px solid #e2e8f0;text-align:center;">
                ...
                <p style="margin:0;font-size:13px;color:#64748b;line-height:1.6;">This cleaner hasn't received any reviews just yet — but rest assured, all cleaners are interviewed before joining the platform. Feel free to ask them about their previous experience.</p>
              </div>
            </td></tr>

            ${message ? `
            <tr><td style="padding:16px 20px;border-top:1px solid #f1f5f9;">
            ...
            <tr><td style="padding:16px 20px 20px;border-top:1px solid #f1f5f9;">
              <table width="100%" cellpadding="0" cellspacing="0"><tr>
