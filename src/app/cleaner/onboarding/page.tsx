  ...
  return (
    <div style={{ marginBottom: '24px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px' }}>
        <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: 'linear-gradient(135deg, #3b82f6, #6366f1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: 700, color: 'white', flexShrink: 0 }}>{step}</div>
...
function CheckToggle({ label, checked, onChange, description }: { label: string; checked: boolean; onChange: (v: boolean) => void; description?: string }) {
  return (
    <button onClick={() => onChange(!checked)} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', width: '100%', background: checked ? '#f0fdf4' : '#f8fafc', border: `1.5px solid ${checked ? '#86efac' : '#e2e8f0'}`, borderRadius: '12px', padding: '14px 16px', cursor: 'pointer', textAlign: 'left', transition: 'all 0.15s', fontFamily: 'inherit' }}>
      <div style={{ width: '20px', height: '20px', borderRadius: '6px', flexShrink: 0, marginTop: '1px', background: checked ? '#16a34a' : 'white', border: `2px solid ${checked ? '#16a34a' : '#cbd5e1'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s' }}>
    ...
    <div style={{ maxWidth: '420px', margin: '0 auto' }}>
      <div style={{ background: 'white', borderRadius: '20px', border: '2px solid #e2e8f0', boxShadow: '0 8px 32px rgba(0,0,0,0.08)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '12px 16px', borderBottom: '1px solid #f1f5f9' }}>
          ...
          <div style={{ textAlign: 'center', marginTop: '8px', fontSize: '12px', color: '#94a3b8' }}>Accept a cleaner's application to view their reviews.</div>
        </div>
        <div style={{ padding: '14px 16px', display: 'flex', gap: '10px' }}>
          <button style={{ flex: 1, padding: '12px', borderRadius: '12px', background: 'linear-gradient(135deg, #22c55e, #16a34a)', color: 'white', border: 'none', fontSize: '13px', fontWeight: 700, cursor: 'default', fontFamily: 'inherit', boxShadow: '0 2px 8px rgba(34,197,94,0.25)' }}>✓ Accept &amp; chat</button>
          <button style={{ flex: 1, padding: '12px', borderRadius: '12px', background: '#f8fafc', color: '#64748b', border: '1.5px solid #e2e8f0', fontSize: '13px', fontWeight: 700, cursor: 'default', fontFamily: 'inherit' }}>✕ Decline &amp; notify</button>
        </div>
      </div>
    </div>
  )
...
export default function CleanerOnboarding() {
  ...
  const set = (key: string, value: any) => {
    setForm(f => {
      ...
      if (key === 'confirm_password') {
        if (value && value !== next.password) setErrors(e => ({ ...e, confirm_password: 'Passwords do not match.' }))
        else setErrors(e => { const n = { ...e }; delete n.confirm_password; return n })
      }
      return next
    })
  ...
  const validate = () => {
    ...
    else if (!isValidName(form.full_name)) e.full_name = 'Name should only contain letters, spaces, hyphens or apostrophes — no numbers.'
    if (!form.email.trim()) e.email = 'Please enter your email address.'
    else if (!isValidEmail(form.email)) e.email = 'Please enter a valid email address (e.g. you@example.com).'
    if (!form.phone.trim()) e.phone = 'Please enter your phone number.'
    else if (!isValidPhone(form.phone)) e.phone = 'Please enter a valid UK phone number (e.g. 07700 900000).'
    if (!form.password) e.password = 'Please set a password.'
    else if (!isValidPassword(form.password)) e.password = 'Password must be at least 8 characters and include one uppercase letter and one number or symbol.'
    if (!form.confirm_password) e.confirm_password = 'Please confirm your password.'
    else if (form.password !== form.confirm_password) e.confirm_password = 'Passwords do not match — please check and try again.'
    if (form.selectedAreas.length === 0) e.selectedAreas = "Please select at least one area you're happy to cover."
    if (!platformAgreement) e.platformAgreement = 'You must read and agree to this before submitting your application.'
    return e
  }
  ...
  const inputStyle = { width: '100%', background: '#f8faff', border: '1.5px solid #e2e8f0', borderRadius: '12px', padding: '12px 14px', fontSize: '14px', color: '#1e293b', fontFamily: 'inherit', boxSizing: 'border-box' as const, outline: 'none' }
        ...
        <div style={{ background: 'rgba(255,255,255,0.82)', backdropFilter: 'blur(16px)', borderRadius: '24px', padding: '40px', boxShadow: '0 4px 24px rgba(0,0,0,0.06)', border: '1.5px solid rgba(255,255,255,0.9)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div id="field-full_name">
              ...
              <input style={errors.full_name ? inputErrorStyle : inputStyle} placeholder="e.g. Sarah Mitchell" value={form.full_name} onChange={e => set('full_name', e.target.value)} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div>
              ...
              <select style={{ ...inputStyle, appearance: 'none' as const, cursor: 'pointer' }} value={form.years_experience} onChange={e => set('years_experience', e.target.value)}>
          ...
          <SectionHeader step={3} title="Credentials & accreditations" subtitle="Vouchee requires all cleaners to have a valid DBS check, right to work in the UK, and public liability insurance. If you don't have everything yet, we can help." />
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div style={{ marginTop: '4px' }}>
                ...
                {missingCredentials && (
                  <div style={{ background: '#fffbeb', border: '1.5px solid #fde68a', borderRadius: '12px', padding: '16px', marginTop: '8px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <p style={{ fontSize: '14px', color: '#92400e', margin: 0, lineHeight: 1.6 }}>No problem. You can still apply and we'll email you a step-by-step guide on how to get your DBS certificate and public liability insurance.</p>
                    <CheckToggle label="Email me a simple step-by-step guide on getting a DBS check and public liability insurance." checked={form.needs_credentials_help} onChange={v => set('needs_credentials_help', v)} />
                  </div>
                )}
              </div>
            )}
          ...
          <div id="field-selectedAreas">
            <SectionHeader step={4} title="Where you work" subtitle="Select the areas of Horsham you'd like to cover. You can update this from your dashboard at any time." />
            <ZoneSelector selectedAreas={form.selectedAreas} onToggle={handleToggleArea} onToggleAll={handleToggleAll} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <CheckToggle label="🔔 Notify me about cover clean requests" checked={form.cover_cleans_notify} onChange={v => set('cover_cleans_notify', v)} description="Cover cleans are one-off jobs where a customer needs cover. These requests often pay more and go quickly." />
            <CheckToggle label="🔔 Notify me about new cleaning requests" checked={form.job_notify} onChange={v => set('job_notify', v)} description="We'll alert you when customers post new cleaning requests in the areas you cover." />
            {form.job_notify && (
              <div style={{ background: '#f0f9ff', border: '1.5px solid #bae6fd', borderRadius: '12px', padding: '12px 16px', display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                ...
                <p style={{ fontSize: '13px', color: '#0369a1', margin: 0, lineHeight: 1.5 }}>You're free to adjust what jobs you're notified for from your dashboard at any time.</p>
              </div>
            )}
            <CheckToggle label="📣 Keep me informed with Vouchee updates and promotions" checked={form.marketing_opt_in} onChange={v => set('marketing_opt_in', v)} description="Occasional emails about your account, platform updates, product launches, and exclusive offers." />
          </div>

          <Divider />

          <SectionHeader step={6} title="Your cleaner card" subtitle="This is exactly what a customer sees when you apply for one of their jobs." />
          <ApplicationCardPreview form={form} />

          <Divider />

          <div id="field-platformAgreement" style={{ marginBottom: '28px' }}>
            <div style={{ background: platformAgreement ? '#f0fdf4' : '#fafafa', border: `2px solid ${errors.platformAgreement ? '#fca5a5' : platformAgreement ? '#86efac' : '#e2e8f0'}`, borderRadius: '16px', padding: '20px', transition: 'all 0.2s' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: '14px' }}>
                <button onClick={() => { setPlatformAgreement(!platformAgreement); if (errors.platformAgreement) setErrors(e => { const next = { ...e }; delete next.platformAgreement; return next }) }} style={{ width: '22px', height: '22px', borderRadius: '6px', flexShrink: 0, marginTop: '2px', background: platformAgreement ? '#16a34a' : 'white', border: `2px solid ${platformAgreement ? '#16a34a' : '#cbd5e1'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 0.15s', fontFamily: 'inherit', padding: 0 }}>
